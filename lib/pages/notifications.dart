  ...
  handleSendNotification() async {
    ...
    if (formKey.currentState.validate()) {
      ...
      if (ab.userType == 'tester') {
        Navigator.pop(context);
        openDialog(
            context, 'You are a Tester', 'Only admin can send notifications');
      } else {
        await getTimestamp()
            .then((value) => context
                .read<NotificationBloc>()
                .sendNotification(titleCtrl.text, descriptionCtrl.text))
            .then((value) => context.read<NotificationBloc>().saveToDatabase(
                timestamp, titleCtrl.text, descriptionCtrl.text))
            .then((value) =>
